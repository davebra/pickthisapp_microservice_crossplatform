...
exports.create = function (req, res) {
    ...
    var dataTagLower = req.body.tag.toLowerCase();
  
    // validation tag if is in the blacklist
    if ( wordblacklist.indexOf(dataTagLower) > -1 ) {
        res.json({
            status: "error",
            message: "Blacklisted",
        });
    ...
    // fill the object
    tag.name = dataTagLower;
    tag.items = 0;

    // save in mongodb the tag and check for errors
